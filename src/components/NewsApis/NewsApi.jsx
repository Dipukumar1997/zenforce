...
export default function NewsApi() {
  ...
  useEffect(() => {
    ...
    fetchRandomNews();
  }, [API_KEY]); // Added API_KEY to dependency array

  // Fetch default/trending news directly from GNews
  const fetchRandomNews = async () => {
    setLoading(true);
    try {
      // Direct GNews API URL - No proxy needed for localhost
      const apiUrl = `https://gnews.io/api/v4/top-headlines?category=general&lang=en&country=us&max=10&apikey=${API_KEY}`;
      
      const response = await fetch(apiUrl);
      const newsData = await response.json();
      
      console.log('GNews Response:', newsData); // Debug
      
      if (newsData.articles) {
        setArticles(newsData.articles);
      } else if (newsData.errors) {
        console.error('GNews API Error:', newsData.errors);
        alert('Error: ' + newsData.errors[0]);
      }
    } catch (error) {
      console.error('Error fetching news:', error);
      alert('Failed to fetch news. Check console for details.');
  ...
  // Fetch news based on search query directly from GNews
  const fetchNewsByQuery = async (query) => {
    setLoading(true);
    try {
      // Direct GNews search URL - No proxy needed
      const apiUrl = `https://gnews.io/api/v4/search?q=${encodeURIComponent(query)}&lang=en&max=10&apikey=${API_KEY}`;
      
      const response = await fetch(apiUrl);
      const newsData = await response.json();
      
      console.log('GNews Search Response:', newsData); // Debug
